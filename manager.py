...
"
    ...
    rows = con.execute(
        text(
            "SELECT p.Name, p.Conference, c.Conference_Strength, p.Team, \
...
def add():
    ...
    if request.method == "POST":
        ...
        session["College_Level_of_Competition"] = request.form[
            "College_Level_of_Competition"
        ]
...
def search():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "/searchResults.html", rows=rows, player_list=player_list
        )
    else:
...
def max():
    ...
    rows = con.execute(
        text(
            "SELECT Conferences.Conference_Name, Player.Name, Player.Score \
                           FROM Player \
                           INNER JOIN Conferences ON Player.Conference = Conferences.Conference_Name \
                           WHERE Player.Score = ( \
                           SELECT MAX(Score) \
                           FROM Player AS p \
                           WHERE p.Conference = Player.Conference) ORDER BY Conferences.Conference_Name;"
        )
    ).fetchall()
    con.close()
